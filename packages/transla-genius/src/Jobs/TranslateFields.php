...
class TranslateFields implements ShouldQueue
{
    ...
    public function handle(AutoTranslationService $translationService)
    {
        try {
            ...
            if (!in_array($currentLocale, $targetLanguages)) {
                $targetLanguages[] = $currentLocale;
            }
    ...
    protected function prepareTranslations(AutoTranslationService $translationService, string $currentLocale, array $targetLanguages): array
    {
        ...
        foreach ($this->fields as $field) {
            ...
            $rawTranslations = $this->model->getTranslations($field);
            $fieldTranslations = $this->normalizeFieldTranslations($rawTranslations, $currentLocale);

            
            $sourceLocale = !empty($fieldTranslations[$currentLocale]) 
                ? $currentLocale 
                : array_key_first($fieldTranslations);

            
            if (empty($sourceLocale) || empty($fieldTranslations[$sourceLocale])) {
                continue;
            }
            ...
            foreach ($targetLanguages as $targetLanguage) {
                if (empty($fieldTranslations[$targetLanguage])) {
                    $translated = $translationService->translate(
                        $fieldTranslations[$sourceLocale],
                        $sourceLocale,
                        $targetLanguage
                    );
